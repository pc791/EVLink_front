export type ChargingStation = {
  id: string; // 각 충전소를 식별할 고유 ID
  position: { lat: number; lng: number };
  addr: string;
  chargeTp: string;
  cpStat: string;
  cpTp: string;
//   cpNm: string;
  // ... 기타 상세 정보adasddasdasdasdsadadas
};

// addr 충전기주소ddd
// chargeTp 충전기타입 1:완속, 2:급속
// cpStat 충전기 상태코드	STRING	1:충전가능 2:충전중 3:고장/점검 4:통신장애 5:통신미연결
// cpTp 충전방식	STRING	1:B타입(5핀) 2:C타입(5핀) 3:BC타입(5핀) 4:BC타입(7핀) 5:C차 데모 6:AC3상 7:DC콤보 8:DC차데모+DC콤보
// cpNm 충전소명칭	STRING	충전소 명칭(ex:한전본사 남측주차장)

// 더미 데이터
export const DUMMY_STATIONS: ChargingStation[] = [
  { id: 'cs1', position: { lat: 37.5009, lng: 127.0366 }, addr: '충전소 A', chargeTp: '급속', cpStat:'충전가능', cpTp:'B타입(5핀)' },
  { id: 'cs2', position: { lat: 37.5020, lng: 127.0420 }, addr: '충전소 B', chargeTp: '완속', cpStat:'충전중', cpTp:'C타입(5핀)' },
  { id: 'cs3', position: { lat: 37.5055, lng: 127.0390 }, addr: '충전소 C', chargeTp: '급속', cpStat:'고장/점검', cpTp:'BC타입(5핀)' },
  { id: 'cs4', position: { lat: 37.5050, lng: 127.0400 }, addr: '충전소 D', chargeTp: '완속', cpStat:'통신장애', cpTp:'DC콤보' },
  // 더미 데이터 시작
  // 아래부터는 랜덤 200개
  ...Array.from({ length: 500 }, (_, i) => {
    const lat = 37.45 + Math.random() * 0.1; // 서울 위도 범위 대략
    const lng = 126.90 + Math.random() * 0.15; // 서울 경도 범위 대략
    const chargeTpList = ['완속', '급속'];
    const cpStatList = ['충전가능', '충전중', '고장/점검', '통신장애', '통신미연결'];
    const cpTpList = [
      'B타입(5핀)',
      'C타입(5핀)',
      'BC타입(5핀)',
      'BC타입(7핀)',
      'C차 데모',
      'AC3상',
      'DC콤보',
      'DC차데모+DC콤보'
    ];
    return {
      id: `cs${i + 5}`,
      position: { lat: parseFloat(lat.toFixed(6)), lng: parseFloat(lng.toFixed(6)) },
      addr: `서울시 충전소 ${i + 5}`,
      chargeTp: chargeTpList[Math.floor(Math.random() * chargeTpList.length)],
      cpStat: cpStatList[Math.floor(Math.random() * cpStatList.length)],
      cpTp: cpTpList[Math.floor(Math.random() * cpTpList.length)]
    };
  })
];